...
vows.run = function(options, callback) {
  return vows.runner.run(options, callback);
};
...
vows.Context = (function() {
  ...
  function Context(description, content, parent) {
    this.callback = __bind(this.callback, this);
    var key, value, _i, _j, _len, _len2, _ref;
    ...
    _ref = ['total', 'running', 'honored', 'pending', 'broken', 'errored'];
      ...
      case 'object':
        if (content.length != null) {
          ...
          for (_j = 0, _len2 = content.length; _j < _len2; _j++) {
            value = content[_j];
            this.add(new vows.Context(null, value, this));
          }
        } else {
          this.type = 'group';
          this.content = {};
          for (key in content) {
            value = content[key];
            if (key === 'topic' || key === 'batch' || key === 'async' || key === 'setup' || key === 'teardown') {
              if (key === 'topic') this.hasTopic = true;
              this[key] = value;
            } else {
              this.add(new vows.Context(key, value, this));
            }
          }
        }
        break;
  ...
  Context.prototype.report = function() {
    if (!this.options.silent) return vows.report.apply(this, arguments);
  };

  Context.prototype._errorPattern = /^function\s*\w*\s*\(\s*(e|err|error)\b/;

  Context.prototype.run = function(topics, options) {
    var batch, child, children, cur, key, next, runChild, _ref, _ref2, _ref3;
    var _this = this;
    ...
    (function() {
      ...
      return _this.env = new ((function() {
        ...
        _Class.prototype = (context.parent ? context.parent.env : {});
    ...
    if (this.parent === vows.runner) {
      if (this.description) this.report('subject', this.description);
    }
    switch (this.type) {
      case 'comment':
        this.end('pending');
      ...
      case 'test':
        ...
        break;
      case 'batch':
        if (!this.content.length) return this.end('done');
        batch = this.content.slice();
        while (batch.length) {
          cur = batch.pop();
          if (typeof next !== "undefined" && next !== null) {
            cur.once('end', (function(next) {
              return function() {
                return next.run(topics, this.options);
              };
            })(next));
          } else {
            cur.once('end', function() {
              return _this.end('done');
            });
          }
          next = cur;
        }
        cur.run(this.topics, this.options);
        break;
      case 'group':
        if (!((function() {
          var _results;
        ...
        this.once('topic', function() {
          var args;
          _this.exception = arguments[0];
          if (_this.hasTopic) {
            args = Array.prototype.slice.call(arguments, 1);
            return _this.topics = args.concat(_this.topics);
          }
        });
        runChild = function(child) {
          _this.results.running++;
          if (child.type === 'test' && _this._errorPattern.test(child.content)) {
            return child.run([_this.exception].concat(_this.topics), _this.options);
          } else if (_this.exception) {
            child.options = _this.options;
            child.exception = _this.exception;
            return child.end('errored');
          } else {
            return child.run(_this.topics, _this.options);
          }
        };
        children = [];
        this.hasTests = false;
        ...
        for (key in _ref3) {
          ...
          if (!this.hasTests && child.type === 'test') {
            ...
            this.once('run', function() {
              var context, parts;
              context = _this;
              parts = [_this.description];
              while ((context = context.parent) && context.parent !== vows.runner) {
                if (context.description) parts.unshift(context.description);
              }
              return _this.report('context', {
                description: parts.join(' ')
              });
            });
          }
          child.once('end', function(result) {
            _this.results.running--;
            if (_this.batch && children.length) {
              return runChild(children.shift());
            } else if (!_this.results.running) {
              return _this.end('done');
            }
          });
        }
        this.once('topic', function() {
          ...
          if (_this.batch) {
            ...
            for (_i = 0, _len = children.length; _i < _len; _i++) {
              child = children[_i];
              _results.push(runChild(child));
            }
            return _results;
          }
        });
        this.once('topic', function() {
          if (_this.teardown != null) {
            return _this.teardown.apply(_this, _this.topics);
          }
        });
        if (!this.hasTopic) {
          if (this.topics.length) this.topic = this.topics[0];
        ...
        if (this.topic != null) {
          if (this.topic instanceof events.EventEmitter) {
            this.async = true;
            this.topic.once('success', function() {
              return _this.success.apply(_this, arguments);
            });
            this.topic.once('error', function() {
  ...
  Context.prototype.end = function(result) {
    ...
    process.nextTick(function() {
      return _this.emit(_this.status = 'end', _this.result);
    });
    return this;
  };
  ...
  Context.prototype.error = function() {
    var args;
    args = Array.prototype.slice.call(arguments);
    if (!args.length) args.unshift(new Error('Unspecified error'));
    return this.callback.apply(this, args);
  };
  ...
  Context.prototype.callback = function() {
    ...
    args = Array.prototype.slice.call(arguments);
    if (typeof args[0] === 'boolean' && args.length === 1) {
      this.emit('topic', null, args[0]);
    } else {
...
vows.Runner = (function() {
  ...
  Runner.prototype.run = function(options, callback) {
    ...
    this.once('end', function() {
      _this.results.dropped = _this.results.total - (_this.results.honored + _this.results.pending + _this.results.errored + _this.results.broken);
      _this.report('finish', _this.results);
      if (callback != null) return callback(_this.results);
    });
    return Runner.__super__.run.call(this, null, this.options);
  };
...
vows.format = function(str) {
  var _this = this;
  str = str.replace(/`([^`]+)`/g, function(_, part) {
    return vows.stylize(part).italic();
  });
  str = str.replace(/\*([^*]+)\*/g, function(_, part) {
    return vows.stylize(part).bold();
  });
  str = str.replace(/_([^_]+)_/g, function(_, str) {
...
vows.stringify = function(obj) {
    ...
    if (s === 'object' || s === 'function') {
      if (value != null) {
        for (_i = 0, _len = types.length; _i < _len; _i++) {
          type = types[_i];
          if (value instanceof type) s = type.name.toLowerCase();
        }
      } else {
        s = 'null';
      }
  ...
  result = (function() {
    switch (typeOf(obj)) {
      ...
      case 'function':
        return vows.stylize('[Function]', 'function');
      case 'date':
      ...
      case 'array':
        pretty = len(obj) > 4 || ((function() {
          var _i, _len, _results;
          _results = [];
          for (_i = 0, _len = obj.length; _i < _len; _i++) {
            o = obj[_i];
            if (len(o) > 0) _results.push(o);
          }
          return _results;
        })()).length;
        start = pretty ? '\n' + ((function() {
...
stylizers.ConsoleStylizer = (function() {
  ...
  _fn = function(style) {
    return ConsoleStylizer.prototype[style] = function() {
...
stylizers.HTMLStylizer = (function() {
  ...
  _fn = function(style) {
    return HTMLStylizer.prototype[style] = function() {
      return this.stylize(style);
    };
  };
...
vows.report = function() {
  if (vows.reporter) return vows.reporter.report.apply(vows.reporter, arguments);
};
...
assert.AssertionError.prototype.toString = function() {
  ...
  if (this.stack) source = this.stack.match(/([a-zA-Z0-9_-]+\.js)(:\d+):\d+/);
  if (this.message) {
    ...
    return vows.stylize([this.expected, this.operator, this.actual].join(' ')).warning().toString();
  }
};
...
assert.isZero = function(actual, message) {
  if (actual !== 0) return assert.fail(actual, 0, message, '===', assert.isZero);
};
  ...
  for (key in defaults) {
    ...
    assert[key] = (function(n, key, defaultMessage, callback) {
      return function() {
        ...
        if ((_ref2 = args[n]) == null) args[n] = defaultMessage;
